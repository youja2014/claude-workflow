...
name: scaffold
description: |
  신규 프로젝트 scaffolding 또는 기존 프로젝트 adoption. 3 스택 (Python CLI/FastAPI,
  TS Nx 모노레포). 신규는 템플릿 복사 + 클린 아키텍처 + Docker. 기존은 컴포넌트별
  옵트인 주입 (.githooks, Makefile, docs/, CLAUDE.md).
  TRIGGER when: 사용자가 `/scaffold ...` 슬래시 호출, "새 프로젝트 만들어줘 — 스택 X"
  형태의 요청, 또는 기존 git repo 안에서 "이 프로젝트에 컨벤션 적용해줘" / `/scaffold`
  (인자 없이) 호출.
  SKIP when: 기존 프로젝트의 단일 파일 수정 요청, 코드 리뷰/디버그/리팩토링 요청,
